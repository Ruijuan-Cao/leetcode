...
impl Solution {
    pub fn solve_sudoku(board: &mut Vec<Vec<char>>) {
        ...
        Self::back_trace(board, 0, 0);
    }
    pub fn back_trace(board: &mut Vec<Vec<char>>, row: usize, column: usize) -> bool {
        if column == board[0].len() {
            return Self::back_trace(board, row + 1, 0);
        }
        if row == board.len() {
            return true;
        }
        if board[row][column] != '.' {
            return Self::back_trace(board, row, column + 1);
        }
        for i in 1..10 {
            let ch = from_digit(i, 10).unwrap();
            if !Self::is_valid_sudoku(&board, row, column, ch) {
                continue;
            }
            board[row][column] = ch;
            if Self::back_trace(board, row, column + 1) {
                return true;
            }
            board[row][column] = '.';
        }
